...
type_colors = c("integer" = "#009FFA", "number" = "#003C86", "string" = "#FFDC3D")
...
for(pmo_class in pmo_schema_json$`$defs`){
  ...
  for(required in pmo_class$required){
    ...
    cat(paste0("			<tr> <td bgcolor=\"", bg, "\">", type, "</td><td bgcolor=\"", list_or_single_value_colors[list_or_single_value], "\">", list_or_single_value, "</td> <td port=\"", required, "\" bgcolor=\"#008607\">", required, "</td> </tr>\n"),file = outputFile, append = T)
  }
  ...
  if(length(non_required_props) > 0){
    ...
    for(non_required in non_required_props){
      type = NA
      list_or_single_value = NA
      if("$ref" %in% names(pmo_class$properties[[non_required]])){
        list_or_single_value = "single"
        type = gsub(".*\\/", "", pmo_class$properties[[non_required]]$`$ref`)
      } else if (pmo_class$properties[[non_required]]$type == "array" && "$ref" %in% names(pmo_class$properties[[non_required]]$items)){
        list_or_single_value = "list"
        type = gsub(".*\\/", "", pmo_class$properties[[non_required]]$items$`$ref`)
      } else if (pmo_class$properties[[non_required]]$type == "array" && "type" %in% names(pmo_class$properties[[non_required]]$items)){
        list_or_single_value = "list"
        type = pmo_class$properties[[non_required]]$items$type
      } else if ("type" %in% names(pmo_class$properties[[non_required]])){
        list_or_single_value = "single"
        type = pmo_class$properties[[non_required]]$type
      }
      bg = "#FFCFE2"
      if(type %in% names(type_colors)){
        bg = as.character(type_colors[type])
      }
      cat(paste0("			<tr> <td bgcolor=\"", bg, "\">", type, "</td><td bgcolor=\"", list_or_single_value_colors[list_or_single_value], "\">", list_or_single_value, "</td> <td port=\"", non_required, "\" bgcolor=\"#F60239\">", non_required, "</td> </tr>\n"), file = outputFile, append = T)
    }
  }
  cat("		</table>>
		shape=plain		
  ...
  for(prop in names(pmo_class$properties)){
    ...
    if("$ref" %in% names(pmo_class$properties[[prop]])){
      cat(paste0("	", pmo_class$title, "_NODE:", prop, " -> ", gsub(".*\\/", "", pmo_class$properties[[prop]]$`$ref`), "_NODE:", gsub(".*\\/", "", pmo_class$properties[[prop]]$`$ref`), "\n"),
          file = outputFile, append = T)
    } else if (pmo_class$properties[[prop]]$type == "array" && "$ref" %in% names(pmo_class$properties[[prop]]$items)){
...
replacing the mermaid plot below 

:::{.column-screen}

```{mermaid, eval = F, echp = F}
%%| fig-align: center 
